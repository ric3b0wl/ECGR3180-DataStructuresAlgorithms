...
// countInv
int merge(vector<int>& nums, vector<int>& left, vector<int>& right) {
	int i = 0, j = 0, k = 0;
  int inversions = 0;
	...
	while ((j < left.size()) && (k < right.size())) {
		if (left[j] <= right[k]) {
			...
			i++; 
			k++; // right index
      inversions += left.size() - j; // add number of remaining elements in left array to inversions
		}
	}

  // take everything that is in the left array and put it in
  // the output array
...
int mergesort(vector<int>&nums) {
  ...
  int left_inversions = 0, right_inversions = 0, current_inversions = 0;
	if (nums.size() > 1) {
		...
		left_inversions = mergesort(left); //AG
		right_inversions = mergesort(right); //AG
		current_inversions = merge(nums, left, right); //AG
	}

  return left_inversions + right_inversions + current_inversions; //current-merge
}
