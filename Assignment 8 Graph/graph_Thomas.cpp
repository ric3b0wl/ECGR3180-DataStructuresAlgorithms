...
vector<int> DFS(const adjlist& alist, int source) {
	...
	vector<int> dfsList; // returning vector
	...
	dfsList.push_back(source);	// Because source is visited, first entry to return is source
	DFSHelper(alist, dfsList, vis, source);	// Now we need the helper for the rest	
	return dfsList; // Return list of all nodes visited
...
vector<int> BFS(const adjlist& alist, int source) {
	// Make a queue, start source in queue
	queue<int> q;
	q.push(source);
	...
	while (!q.empty()){
		auto w = q.front(); // Take first node w
		q.pop();			// pop the queue
		for (auto& pair : alist[w]){ // adjacent nodes at node w
			auto v = pair.first;	// first adjacent node v
 			if (!vis[v]){
					vis[v] = true;	// mark v as visited
					q.push(v);		// put back into queue, important
					bfsList.push_back(v);	// v is visted now
			}
		}
	}
